...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T13:44:00+00:00",
...
此筆記本展示了使用 Microsoft Agent Framework for .NET 和 GitHub 模型進行**順序工作流程模式**。您將學習如何構建複雜的逐步處理管道，代理按照特定順序執行，每個階段基於前一階段的結果進行構建。
...
讓我們一起構建穩健的順序 AI 處理管道吧！🚀

## 💻 運行代碼

完整的實現可在 `02.dotnet-agent-framework-workflow-ghmodel-sequential.cs` 文件中找到。此文件展示了一個**三階段家具分析工作流程**：
...
順序中的每個代理接收：
- **原始輸入**：初始用戶消息（圖片 + 文本）
- **前一代理輸出**：對話歷史中所有前一代理的響應
- **累積上下文**：在整個工作流程中保持的完整狀態

這使得每個代理能夠基於所有前一階段的完整上下文進行複雜的多階段處理。
