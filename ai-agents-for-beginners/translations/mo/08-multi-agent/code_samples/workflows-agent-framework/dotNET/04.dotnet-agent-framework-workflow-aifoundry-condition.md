...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T12:05:11+00:00",
...
**主要組件：**
- **Draft Executor**：從大綱生成初始內容草稿的 AI 代理
- **Content Review Executor**：評估草稿質量和合規性的 AI 代理
...
完整實現可在 `04.dotnet-agent-framework-workflow-aifoundry-condition.cs` 中找到。此示例展示了**具有質量門檻的內容生成工作流程**：
...
**工作流程中的代理：**
1. **Evangelist Agent**：使用 Bing 基礎信息從大綱生成教程草稿
...
2. **生成草稿**：Evangelist 代理從大綱生成教程草稿
3. **審核內容**：Content Reviewer 評估草稿質量
4. **條件式路由**：
   - **若批准（>200 字）**：Publish 執行器保存為 Markdown 文件
   - **若拒絕（<200 字）**：SendReview 執行器發送審核通知
5. **顯示結果**：展示最終工作流程結果
