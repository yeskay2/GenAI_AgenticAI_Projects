...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T13:09:23+00:00",
...
完整的實現可在 `03.dotnet-agent-framework-workflow-ghmodel-concurrent.cs` 文件中找到。此文件展示了一個用於旅行規劃的 **分散/集中並行工作流程**：
...
1. **ConcurrentStartExecutor**：將用戶請求同時廣播給所有代理
...
工作流程將：
1. **廣播請求**：將「計劃一次12月去西雅圖的旅行」發送給兩個代理
2. **並行處理**：兩個代理同時工作：
   - Researcher 分析景點和細節
   - Planner 創建行程和物流計劃
3. **聚合**：合併兩個響應為綜合輸出
4. **顯示結果**：展示包含所有信息的綜合旅行計劃
