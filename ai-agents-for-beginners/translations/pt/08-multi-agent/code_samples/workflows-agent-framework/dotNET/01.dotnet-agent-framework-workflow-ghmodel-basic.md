...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T12:47:25+00:00",
...
Este notebook demonstra como construir fluxos de trabalho sofisticados de **agentes** utilizando o Microsoft Agent Framework para .NET e Modelos GitHub. Aprenderá a criar processos empresariais de múltiplas etapas onde agentes de IA colaboram para realizar tarefas complexas através de padrões estruturados de orquestração.
...
1. Inscreva-se nos Modelos GitHub (atualmente em pré-visualização)
2. Gere um token de acesso pessoal com permissões de acesso ao modelo
...
- **Privacidade de Dados**: Tratamento seguro de informações sensíveis ao longo dos fluxos de trabalho

### 📊 **Observabilidade e Gestão**
- **Design Visual de Fluxos de Trabalho**: Representação clara dos fluxos de processo e dependências
- **Monitoramento de Execução**: Acompanhamento em tempo real do progresso e desempenho dos fluxos de trabalho
...
Vamos construir o seu primeiro fluxo de trabalho de IA pronto para empresas! 🚀

## 💻 Executando o Código

A implementação completa está disponível em `01.dotnet-agent-framework-workflow-ghmodel-basic.cs`. Este arquivo demonstra:

1. **Configuração do Ambiente** - Carregando credenciais dos Modelos GitHub a partir do arquivo `.env`
2. **Configuração do Cliente OpenAI** - Configurando o cliente para usar o endpoint dos Modelos GitHub
3. **Criação de Agentes** - Definindo agentes especializados (Recepção e Concierge)
4. **Workflow Builder** - Criando um fluxo de trabalho com múltiplos agentes e processamento sequencial
...
1. Aceitar o seu pedido de destino de viagem ("Gostaria de ir para Paris")
...
- Modificando as instruções dos agentes para alterar o comportamento deles
- Adicionando mais agentes para criar fluxos de trabalho complexos e de múltiplas etapas
