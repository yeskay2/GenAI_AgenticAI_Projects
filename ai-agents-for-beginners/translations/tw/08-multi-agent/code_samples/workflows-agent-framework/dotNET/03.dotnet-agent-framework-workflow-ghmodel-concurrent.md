...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T13:10:30+00:00",
...
此筆記本展示了使用 Microsoft Agent Framework for .NET 和 GitHub 模型的**並行工作流程模式**。您將學習如何建立高效能的並行處理工作流程，通過同時執行多個 AI 代理來最大化吞吐量，同時保持協調和數據一致性。

## 🎯 學習目標

### 🚀 **並行處理基礎**
- **代理並行執行**：同時運行多個 AI 代理以獲得最大效能
- **Async/Await 模式**：利用 .NET 的異步程式設計模型進行高效並行處理
- **GitHub 模型整合**：協調對 GitHub AI 模型推理服務的多個並行調用
...
### 📊 **效能監控**
- **並行執行指標**：追蹤所有並行操作的效能
- **資源使用分析**：監控 CPU、內存和網絡利用率
- **吞吐量分析**：衡量並行處理帶來的效率提升
- **瓶頸檢測**：識別並解決效能限制

### 🔧 **開發與運營**
- **異步程式設計模型**：利用 .NET 成熟的 async/await 模式
- **任務協調**：內建的任務管理和協調能力
- **異常處理**：全面的並行操作錯誤處理
...
完整實現可在 `03.dotnet-agent-framework-workflow-ghmodel-concurrent.cs` 文件中找到。此文件展示了一個**扇出/扇入並行工作流程**用於旅行規劃：
...
4. **ConcurrentAggregationExecutor**：收集並合併來自兩個代理的結果

### 🎯 **扇出/扇入模式**

此工作流程展示了經典的**扇出/扇入**模式：
- **扇出**：一條輸入消息同時廣播至多個代理
- **並行處理**：多個代理並行處理相同任務
- **扇入**：收集所有代理的結果並合併為單一輸出
...
此並行工作流程模式是建立高效能、可擴展 AI 代理系統的關鍵！
