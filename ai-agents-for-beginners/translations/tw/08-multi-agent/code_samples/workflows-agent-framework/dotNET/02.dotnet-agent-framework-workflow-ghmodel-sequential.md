...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T13:44:29+00:00",
...
本筆記本展示了使用 Microsoft Agent Framework for .NET 和 GitHub 模型的**順序工作流程模式**。您將學習如何構建複雜的逐步處理管道，代理按照特定順序執行，每個階段基於前一階段的結果進行構建。
...
- **質量保證工作流程**：順序審核、驗證和批准流程
- **內容生產管道**：研究 → 撰寫 → 編輯 → 審核 → 發佈
...
完整的實現可在 `02.dotnet-agent-framework-workflow-ghmodel-sequential.cs` 文件中找到。該文件展示了一個**三階段家具分析工作流程**：
...
順序中的每個代理接收：
- **原始輸入**：初始用戶消息（圖片 + 文本）
- **前一代理的輸出**：對話歷史中所有前一代理的響應
- **累積上下文**：在整個工作流程中保持的完整狀態

這使得每個代理能夠基於所有前一階段的綜合上下文進行複雜的多階段處理。
